    ...
    if hasattr(request.user, 'profile'):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                info = "User Information: From user\n NAME: %s\t%s \n PAN: %s \n PHONE: %s \n DOB: %s \n" \
                       % (request.user.profile.first_name, request.user.profile.last_name, request.user.profile.pan,
                          request.user.profile.phone,
                          request.user.profile.dob)
                main_message = info + ("ASSESSMENT YEAR %s\n" % yr) + contact_message
                from_email = request.user
                to_email = [from_email, 'vamagithub@gmail.com']
                subject = "EasyReturn Form-16 Submission From - %s " % request.user
